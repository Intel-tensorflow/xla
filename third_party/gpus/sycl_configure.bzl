...
def _create_dummy_repository(
    ...
    _tpl(
        ...
        "sycl:BUILD",
        {
            # New placeholders: each expands to full item or "".
            "%{mkl_intel_ilp64_src}": "",
            "%{mkl_sequential_src}": "",
            "%{mkl_core_src}": "",
            "%{mkl_sycl_srcs}": "",
...
def _create_local_sycl_repository(repository_ctx):
    ...
    "%{mkl_sycl_srcs}": mkl_sycl_libs,  # already built as '"..",\n"..",\n...'

    # Old placeholders (safe to leave; template won’t use them after edit)
    ...
    "%{level_zero_libs}": level_zero_libs,            # already '"..",\n'
